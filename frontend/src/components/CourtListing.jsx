...
const CourtListing = () => {
    ...
    let filteredFacilities = facilities.filter(facility => {
        ...
        const minPrice = facility.courts && facility.courts.length > 0 ? Math.min(...facility.courts.map((court) => court.pricePerHour)) : 0;
        ...
        const facilityReviews = reviews ? reviews.filter((r) => r.facilityId === facility._id) : [];
        const rating = facilityReviews.length > 0 ? (facilityReviews.reduce((acc, r) => acc + r.rating, 0) / facilityReviews.length) : 0;
    ...
    if (sortBy === 'Price Low to High') {
        filteredFacilities = filteredFacilities.sort((a, b) => {
            const aMin = a.courts && a.courts.length > 0 ? Math.min(...a.courts.map((court) => court.pricePerHour)) : 0;
            const bMin = b.courts && b.courts.length > 0 ? Math.min(...b.courts.map((court) => court.pricePerHour)) : 0;
            return aMin - bMin;
        });
        ...
        filteredFacilities = filteredFacilities.sort((a, b) => {
            const aReviews = reviews ? reviews.filter((r) => r.facilityId === a._id) : [];
            const bReviews = reviews ? reviews.filter((r) => r.facilityId === b._id) : [];
            const aRating = aReviews.length > 0 ? (aReviews.reduce((acc, r) => acc + r.rating, 0) / aReviews.length) : 4.5;
            const bRating = bReviews.length > 0 ? (bReviews.reduce((acc, r) => acc + r.rating, 0) / bReviews.length) : 4.5;
